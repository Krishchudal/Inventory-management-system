...
public class ItemService
{
    ...
    public static List<ItemHistory> HandleTakeAction(Guid RecordId, Guid actionId, ActionType action)
    {
        ...
        if (action == ActionType.Approve)
        {
            ...
            if (DateTime.Parse("6: 00 AM") >= DateTime.Now || DateTime.Parse("10: 00 PM") <= DateTime.Now)
            {
                throw new Exception(message);
            }

            if (DateTime.Now.DayOfWeek == DayOfWeek.Saturday || DateTime.Now.DayOfWeek == DayOfWeek.Tuesday)
            {
                throw new Exception(message);
            }
